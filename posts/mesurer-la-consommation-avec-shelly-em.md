...
- électricité consommée par [la machine à laver]({{ '/tests/machine-a-laver/' | url }}) et/ou [le sèche-linge]({{ '/tests/seche-linge-a-pompe-a-chaleur/' | url }}). Ici c'était nettement une machine à laver, on reconnait à la forme du cycle, avec une forte consommation au début pour chauffer l'eau.
